...
def main():
    ...
    try:
        ...
        # Получаем параметры из переменных окружения (для cron)
        quarter_offset = int(os.getenv('QUARTER_OFFSET', '-1'))

        q_start, q_end = get_quarter_dates(quarter_offset=quarter_offset)
        pq_start, pq_end = get_quarter_dates(quarter_offset=quarter_offset - 1)
        year, q_num = q_start.year, get_quarter_number(q_start)

        logger.info(f"Генерируем отчет за квартал: {year}Q{q_num} (offset: {quarter_offset})")
        logger.info(f"Период: {q_start.strftime('%d.%m.%Y')} - {q_end.strftime('%d.%m.%Y')}")
        ...
        # Если нет данных
        if not curr_data:
            logger.warning(f"Нет данных за период {q_start.strftime('%d.%m.%Y')} - {q_end.strftime('%d.%m.%Y')}")
            logger.info("ГЕНЕРАЦИЯ ОТЧЕТОВ ЗАВЕРШЕНА (без данных)")
            db.close()
            return

        # 1. Генерация Excel файла
        excel_file = create_excel_from_json_data(curr_data, OUTPUT_DIR, year, q_num)

        if excel_file:
            logger.info(f"Excel файл успешно создан: {excel_file}")

            # Загружаем Excel файл в Bitrix24
...
"""
        ...
        logger.info(
            ...
            f"- Всего анкет в текущем квартале: {len(curr_data)}\n"
            f"- Успешно извлечено оценок: {len(valid_ratings)}\n"
            f"- eNPS компании: {c_stats['enps']}\n"
            f"- Количество промоутеров (9-10): {c_stats['promoters']}\n"
            f"- Количество нейтралов (7-8): {c_stats['neutral']}\n"
            f"- Количество критиков (1-6): {c_stats['detractors']}\n"
            f"- Всего создано отчетов: {total_reports}\n"
            f"- Успешно загружено в Bitrix24: {successful_bitrix_uploads}\n"
            f"- Не загружено в Bitrix24: {failed_bitrix_uploads}")
